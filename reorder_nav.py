...
portage_path = os.path.join(BASE_PATH, 'portage.html')
diario_path = os.path.join(BASE_PATH, 'diario.html')

if os.path.exists(index_path) and os.path.exists(diario_path):
    shutil.move(index_path, portage_path)
    shutil.move(diario_path, index_path)
    print("Files renamed successfully.")

FILES = ['index.html', 'spm.html', 'resumo.html', 'portage.html']
...
def get_bottom_nav(active_page):
    ...
    # Desired Order: Diário -> Resumo -> Portage -> SPM
    # Links: Diário is now index.html, Portage is now portage.html
    return f"""
...
for f in FILES:
    ...
    # Determine the "active_page" for the bottom nav based on filename
    ...
    # Inject new bottom nav before </body>
    bot = get_bottom_nav(active_page)
    content = content.replace('</body>', f'{bot}\n</body>')

    # Update any standalone hrefs in the body (e.g. any manual links, if any)
    # Be careful not to replace the ones we just injected. But we inject bot AFTER this replacement.
    # Actually wait, we should do href replacement FIRST.
    # Since we swapped index and diario, we need to do it via a temp name to avoid collisions.
    # index.html -> TEMP
    # diario.html -> index.html
    # TEMP -> portage.html
    # In regex:
    # First, let's remove the bottom nav so its links aren't affected by our generic replace.
    # Then we do the generic replacements for any other links in the document.
    
    content = content.replace('href="index.html"', 'href="TEMP.html"')
    content = content.replace('href="diario.html"', 'href="index.html"')
    content = content.replace('href="TEMP.html"', 'href="portage.html"')

    with open(filepath, 'w', encoding='utf-8') as file:
        file.write(content)

print("Nav reordered and files renamed successfully.")
